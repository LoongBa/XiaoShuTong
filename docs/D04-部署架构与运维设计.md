...
| 层 | 选型 | 版本 | 说明 |
|----|------|------|------|
| 前端框架（H5 + PC） | **React + shadcn/ui** | React 19 / shadcn/ui（tailwind + radix） | **H5 先行**（学生/家长端）；PC（群主教研端）复用同栈（ADR-007） |
| 构建工具 | Vite | 7.x | H5 构建 |
| 前端状态 | Zustand（轻量）/ TanStack Query | - | 学习会话/记忆状态/服务端缓存 |
| 小程序前端（延后） | **Taro**（React 语法多端） | Taro 4.x | 微信接入开放后接续，复用 React 组件（ADR-007） |
| 管理端（后续可选） | Blazor Wasm | .NET 10 | 后台内部使用，简化开发（ADR-007 保留选项） |
| 后端框架 | **TKWF（团队内部框架）** | 领域 + WebApi + Wasm 三层项目结构 | Web API + SignalR；复用团队既有骨架，不自行搭建 |
| ORM | EF Core | 10.x | PostgreSQL 驱动 |
| 数据库 | **PostgreSQL** | 16.x | JSONB + RLS + 分区 |
| 缓存 | **Redis** | 7.x | 会话/复习队列/限流/热数据 |
| AI 判题 | OpenAI API 兼容接口（LLM 不限） | - | 判题/提示/点评三 Prompt 域；供应商可插拔（火山引擎 Ark / DeepSeek / 智谱等） |
| 任务调度 | Hangfire | - | 复习提醒/过期任务/周报生成 |
| 微信集成 | 微信开放平台 H5 | - | 授权登录/分享（**接入后启用**） |
...
graph TB
    ...
    subgraph External["外部服务"]
        WECHAT["微信开放平台"]
        VOLC["OpenAI 兼容 LLM（火山/DeepSeek 等）"]
        OSS["阿里云 OSS"]
        SLS["日志服务 SLS"]
    end
...
XiaoShuTong.Infrastructure # EF Core/Redis/LLM 客户端（OpenAI 兼容）
```
...
### 5.4 OSS 导出文件生命周期（CSV 内测名单）

| 项 | 配置 |
|----|------|
| 静态资源 | 前端构建产物/题库音频图片 → 长期保留（私有 bucket + 签名 URL，见 §4.1 静态托管） |
| **CSV 导出文件** | 内测名单导出：仅含**手机号后四位 + 一次性邀请码**（D02 §4.17-4.18）→ 存 OSS（`RosterImports.CsvFileUrl`，私有 bucket + 签名 URL）；**建议 7 天过期删除**（`CsvExpiresAt`，对齐 D02 V1.4 §4.18），过期不保留副本 |
| 过期清理 | Hangfire 定时任务扫描 `CsvExpiresAt` 过期记录 → 删除 OSS 文件（名单敏感数据不留存） |
...
## 七、AI 服务接入（OpenAI API 兼容接口）

### 7.1 调用架构

```
判题请求 → 本地关键词引擎（≥85% 直接返回，零成本）
        → 未命中 → OpenAI 兼容大模型 API（带对应 Prompt）
...
| LLM 调用量 / 成本 / 超时率 | LLM 客户端（OpenAI 兼容） |
| 缓存命中率 | Redis |
...
- 审计日志：作答/状态迁移/数据导出/注销/名单导入/CSV 导出/内测码激活操作独立留存
...
| D4-P5 | OpenAI 兼容 LLM 接入 + 成本监控 + 告警全量 |
...
| 2026-09-05 | V1.2 | 技术栈对齐 ADR-007：前端改 React + shadcn/ui（H5 先行 + PC 复用）；后端 .NET 8 → .NET 10；小程序延后 Taro；管理端预留 Blazor Wasm | ADR-007 |
| 2026-09-06 | V1.3 | 内测名单导出对齐（D02 V1.4 §4.18）：新增 §5.4 OSS 导出文件生命周期（CSV 仅手机号后四位+一次性邀请码，7 天过期删除）；§9.3 审计日志补事件类型（名单导入/CSV 导出/内测码激活）；术语对齐：老师教研端→群主教研端 | - |
