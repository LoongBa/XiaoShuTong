...
|:----:|------|-------------|------|---------|
| 0 | **跨切片依赖启动检查**：确认切片 01（群组）`Entities/Groups/*.cs` + 切片 02（学习）`Entities/Learning/*.cs` 已编码；缺失则先执行前序切片编码序列 | - | 依赖就绪 | `.g.cs` 存在 |
...
| 状态 | 动作 |
|------|------|
| 本切片全部步骤完成 + 测试通过 | 向用户报告完成摘要（Entity/Service/测试数 + 编译测试结果）→ **询问用户：是否继续执行下一个切片（切片 04（家校任务闭环域））？** |
...
| 2 | 切片 02（学习Session域）已实施：`Entities/Learning/*.cs` 存在 | |
...
- 后续切片 Agent 读本 HANDOVER §五 步骤 0 即知本切片已实施（`Entities/Bank+Judging/*.cs` 存在）
