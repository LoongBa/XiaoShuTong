...
| 状态 | 动作 |
|------|------|
| 本切片全部步骤完成 + 测试通过 | 向用户报告完成摘要（Entity/Service/测试数 + 编译测试结果）→ **询问用户：是否继续执行下一个切片（切片 08（家长报告订阅域））？** |
...
| 2 | **检查本切片的前序切片实体是否已编码**（逐项）：<br>| 1 | 切片 03（题库判题域）判题引擎已实施：`Services/Judging/JudgingEngineService.cs` 存在 |
| 2 | 切片 06（排行榜搭子域）已实施：`Entities/Buddy/StudyBuddies.cs` 存在 | |
...
- 后续切片 Agent 读本 HANDOVER §五 步骤 0 即知本切片已实施（`Entities/Pk/*.cs` 存在）
