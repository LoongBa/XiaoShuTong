...
|:----:|------|-------------|------|---------|
| 0 | **跨切片依赖启动检查**：确认切片 02（学习）`Entities/Learning/*.cs` + 切片 03（题库判题）`Entities/Bank/*.cs`/`Entities/Judging/*.cs` 已编码；缺失则先执行前序切片编码序列 | - | 依赖就绪 | `.g.cs` 存在 |
...
| 状态 | 动作 |
|------|------|
| 本切片全部步骤完成 + 测试通过 | 向用户报告完成摘要（Entity/Service/测试数 + 编译测试结果）→ **询问用户：是否继续执行下一个切片（切片 05（可视化激励域））？** |
...
| 2 | 切片 03（题库判题域）已实施：`Entities/Bank/*.cs` + `Entities/Judging/*.cs` 存在 | |
...
- 后续切片 Agent 读本 HANDOVER §五 步骤 0 即知本切片已实施（`Entities/Tasks/*.cs` 存在）
...
> *文档版本：第一阶段 v1.1*
> *编制日期：2026-09-06*
> *同步版本：R01 v1.0 / S01 v1.0 / DS01 v1.0 / U01 v1.1*