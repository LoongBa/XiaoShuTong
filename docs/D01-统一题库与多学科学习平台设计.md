...
### 7.6 DB 元数据表（Banks / Questions，V1.3 新增）

> **内容权威仍为 JSON 文件**（§7.2/§7.3 schema，批量校验 §7.4）；DB 表为 **API 查询索引**（列表/取题/知识点筛选/防爬"背一题取一题"）。题目展示内容镜像**不含答案**——答案仅在 JSON 文件与判题服务端，永不下发客户端。命名对齐 D02 V1.4（PascalCase、IX_* 索引）。

#### Banks（题库元数据表，对应 bank.v1.json 的 DB 索引）

```sql
CREATE TABLE Banks (
    Id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    BankId      varchar(64) UNIQUE NOT NULL,   -- 业务键（chinese-7to9-pep）
    Name        varchar(128) NOT NULL,          -- 题库名（七-九年级-统编教材）
    Subject     varchar(16) NOT NULL,           -- 学科（chinese/history/...）
    Version     varchar(16) NOT NULL,           -- 内容版本（V1.0）
    Purpose     varchar(16) NOT NULL DEFAULT 'memorize', -- memorize/assess/play
    Privacy     varchar(16) NOT NULL DEFAULT 'private',  -- private/public/group
    OwnerId     uuid REFERENCES Users(Id),      -- 私域题库 owner（官方题库 NULL）
    JsonPath    varchar(256) NOT NULL,          -- bank.v1.json 文件路径（内容权威）
    Tags        jsonb,                          -- 标签（JSONB，如 ["高频考点"]）
    Status      varchar(16) NOT NULL DEFAULT 'active', -- active/hidden/archived
    CreatedAt   timestamptz NOT NULL DEFAULT now(),
    UpdatedAt   timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IX_Banks_Subject ON Banks(Subject);
CREATE INDEX IX_Banks_OwnerId ON Banks(OwnerId) WHERE OwnerId IS NOT NULL;
```

#### Questions（题目索引表，对应 question 内容的 DB 索引）

```sql
CREATE TABLE Questions (
    Id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    QuestionId       varchar(64) UNIQUE NOT NULL,  -- 业务键（Q-ch-7a-0001）
    BankId           varchar(64) NOT NULL REFERENCES Banks(BankId),
    ChapterId        varchar(64),                  -- 章节/单元（如 7a；Chapters 合并入本字段）
    QType            varchar(8) NOT NULL,          -- R1/R2/R3a/R3b/O1...（D01 题型键）
    Content          jsonb NOT NULL,               -- 题目展示内容镜像（不含答案）
    Keywords         jsonb NOT NULL,               -- KeywordGroup[]（判题关键词，D01 6.1）
    KnowledgePoints  jsonb,                        -- 知识点列表（["观沧海"]）
    Difficulty       smallint NOT NULL DEFAULT 0,  -- 难度 0-5
    Status           varchar(16) NOT NULL DEFAULT 'active', -- active/superseded/hidden
    SupersededBy     varchar(64),                  -- 题目改版替换（软删除，D02 原则 #5）
    CreatedAt        timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IX_Questions_BankId          ON Questions(BankId);
CREATE INDEX IX_Questions_BankId_Chapter  ON Questions(BankId, ChapterId);
CREATE INDEX IX_Questions_QType           ON Questions(QType);
CREATE INDEX IX_Questions_KnowledgePoints ON Questions USING GIN (KnowledgePoints);
```

**同步策略**：
- 导入管线（§7.4 txt/json → schema 校验）成功后：写 JSON 文件（权威）+ **upsert Banks/Questions 元数据行**（事务内）
- 官方题库初始化：批量导入脚本建全量索引；私域题库（PGC/群主上传）入库走同一管线
- **防爬对齐**：`Content` 不含答案，答案仅判题服务端从 JSON 文件读取（D03 §4.5/5.2 契约）
- **RLS**：`privacy=private` 题库按 `OwnerId` 行级安全；官方题库只读（D06）
...
| 2026-09-06 | V1.3 | **新增 §7.6 DB 元数据表（Banks/Questions）**：题库/题目 DB 索引 DDL（PascalCase、IX_*、GIN 知识点索引），content 权威仍为 JSON 文件、镜像不含答案（防爬对齐 D03 契约）；导入管线补"写 JSON 权威 + upsert 元数据"同步策略；Banks 补 OwnerId 私域 RLS | - |
